...
create_plot <- function(df, plot_variables, x_title, metadata, plot_na = FALSE, response_to_sort = NULL, response_order = NULL, variable_order = NULL) {
  ...
  data_long <- df %>%
    ...
    # wrap strings for better formatting in plot, if not already have /n formatting
    mutate(Question = ifelse(!grepl("\n", Question), str_wrap(Question, width = 20), Question),
           Response = ifelse(!grepl("\n", Response), str_wrap(Response, width = 40), Response)
           )
  ...
  if (!missing(variable_order)) {
    ...
    lookup_vector <- setNames(metadata$Question, metadata$name)
