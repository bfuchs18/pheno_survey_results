...
create_plot <- function(df, plot_variables, x_title, metadata, plot_na = FALSE, response_to_sort = NULL, response_order = NULL, variable_order = NULL) {
  ...
  if (!missing(response_order)) { 
      print(paste("response_order:", response_order))
    
    data_long <- data_long %>%
      ### apply str_wrap to match formatting applied to Response above
      mutate(Response = factor(Response, levels = str_wrap(response_order, width = 40)))
  }
  
  # impacts variable order on x-axis
  if (!missing(response_to_sort)) { 
      print(paste("response_to_sort:", response_to_sort))
  ...
  if (!missing(variable_order)) {
    print(paste("variable_order:", variable_order))
...
sfari_datasets_plot <- survey %>%
  ...
  ggplot(., aes(x = study, y = n)) +
    geom_bar(stat = "identity", fill = "steelblue") +
  labs(x = "Study", y = "Count", title = "SFARI pheno dataset(s) worked with (select all that apply)") +
    theme_minimal() + 
...
barplot_data_roles <- survey %>%
  ...
  ggplot(., aes(x = role, y = n)) +
    geom_bar(stat = "identity", fill = "steelblue") +
  labs(x = "role", y = "Count", title = "Role(s) when working with SFARI pheno dataset (select all that apply)") +
  theme_minimal() + 
  scale_x_discrete(
    labels = c(
      "pheno_roles_pi" = "PI",
